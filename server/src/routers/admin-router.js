...
adminRouter.post(
  ...
  async function (req, res, next) {
    try {
      ...
      const userToken = await adminService.getUserToken({ id, pw });
...
// 사용자 정보 수정
// (예를 들어 /admin/users/abc12345 로 요청하면 req.params.userId는 'abc12345' 문자열로 됨)
adminRouter.patch(
  ...
  async function (req, res, next) {
    try {
      ...
      // 사용자 정보를 업데이트함.
      const updatedUserInfo = await adminService.setUser(
        userInfoRequired,
        toUpdate
      );
...
adminRouter.delete(
  ...
  async function (req, res, next) {
    try {
      ...
      const deleteuser = await adminService.DeleteUser(
        userId,
        userPassword,
        inputPassword
      );
...
adminRouter.get(
  ...
  async function (req, res, next) {
    try {
      // 전체 사용자 목록을 얻음
      const userId = req.params.userId;
      const users = await adminService.getUser(userId);
      // 사용자 목록(배열)을 JSON 형태로 프론트에 보냄
      res.status(200).json(users);
    } catch (error) {
      next(error);
